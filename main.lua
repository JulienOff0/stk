...
local S = {
	...
	con = {}, humanoid=nil, normalSpeed=nil
}

-- ===== UI =====
local UI = Instance.new("ScreenGui")
UI.Name = "Premium_Menu_UI"
safeParent(UI)
...
local function makeToggle(parent, defaultOn)
	...
	Btn.Position = UDim2.new(1,-10,0,8)
...
local function makeSlider(parent, title, minV, maxV, defaultV, onChanged)
	...
	return { setValue = function(v) v = math.clamp(v,minV,maxV); local r=(v-minV)/(maxV-minV); setFromX(Wrap.AbsoluteSize.X * r) end }
end
...
-- ===== ESP (version intacte, patch TeamColor) =====
_G.WRDESPEnabled    = false
_G.WRDESPBoxes      = S.esp.Boxes
_G.WRDESPTeamColors = S.esp.TeamColors
_G.WRDESPTracers    = S.esp.Tracers
...
if not _G.WRDESPLoaded then    
    local ESP = {
        ...
        FaceCamera = false, Names = true, TeamColor = true,
        Thickness = 2, AttachShift = 1,
        TeamMates = true, Players = true,
    ...
    function ESP:GetTeam(p) local ov=self.Overrides.GetTeam if ov then return ov(p) end return p and p.Team end
    function ESP:IsTeamMate(p) local ov=self.Overrides.IsTeamMate if ov then return ov(p) end return self:GetTeam(p) == self:GetTeam(plr) end
    function ESP:GetColor(obj) local ov=self.Overrides.GetColor if ov then return ov(obj) end local p=self:GetPlrFromChar(obj) return p and self.TeamColor and p.Team and p.Team.TeamColor.Color or self.Color end
    function ESP:GetPlrFromChar(char) local ov=self.Overrides.GetPlrFromChar if ov then return ov(char) end return plrs:GetPlayerFromCharacter(char) end
    function ESP:Toggle(bool) self.Enabled = bool if not bool then for _,v in pairs(self.Objects) do if v.Type=="Box" then for _,c in pairs(v.Components) do c.Visible=false end end end end end
    ...
    function boxBase:Update()
        ...
        for _,c in pairs(self.Components) do c.Visible=false end
        ...
        if ESP.Boxes and self.Components.Quad then
            ...
            if Vis1 or Vis2 or Vis3 or Vis4 then
                ...
                q.Visible=true
                q.PointA=Vector2.new(TR.X,TR.Y)
                q.PointB=Vector2.new(TL.X,TL.Y)
                q.PointC=Vector2.new(BL.X,BL.Y)
                q.PointD=Vector2.new(BR.X,BR.Y)
                q.Color=color
            end
        end
        ...
        if ESP.Names then
            ...
            if Vis then
                local n=self.Components.Name
                n.Visible=true; n.Position=Vector2.new(Tag.X,Tag.Y); n.Text=self.Name; n.Color=color
                local d=self.Components.Distance
                d.Visible=true; d.Position=Vector2.new(Tag.X,Tag.Y+14); d.Text=math.floor((cam.CFrame.p - cf.p).Magnitude).."m"; d.Color=color
            end
        end
        ...
        if ESP.Tracers then
            ...
            if Vis then
                local t=self.Components.Tracer
                t.Visible=true; t.From=Vector2.new(Torso.X,Torso.Y); t.To=Vector2.new(cam.ViewportSize.X/2, cam.ViewportSize.Y/ESP.AttachShift); t.Color=color
            end
        end
    end
    ...
    function ESP:Add(obj, options)
        ...
        local box=setmetatable({
            ...
            Type="Box", Color=options.Color, Size=options.Size or self.BoxSize,
            Object=obj, Player=options.Player or plrs:GetPlayerFromCharacter(obj),
    ...
    local function CharAdded(char)
        ...
        if hrp then ESP:Add(char,{Name=p.Name, Player=p, PrimaryPart=hrp})
        else
            local ev; ev = char.ChildAdded:Connect(function(c)
                if c.Name=="HumanoidRootPart" then ev:Disconnect(); ESP:Add(char,{Name=p.Name, Player=p, PrimaryPart=c}) end
            end)
        end
    end
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        ...
        if ESP.Enabled then
            for _,v in pairs(ESP.Objects) do
                if v.Update then local ok,err=pcall(v.Update, v) if not ok then warn("[ESP]",err) end end
            end
        end
    end)
